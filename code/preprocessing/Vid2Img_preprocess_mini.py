...
random.seed(42)
for split in ["train"]:
    vid_path_inp = Path(Path.cwd()) / "data/Videos/Greenscreen_Video_frames_4sec_mini" / split / "Input"
    vid_path_lbl = Path(Path.cwd()) / "data/Videos/Greenscreen_Video_frames_4sec_mini" / split / "labels"
    video_names = [vid.stem for vid in vid_path_inp.glob("*")]
    random.shuffle(video_names)
    output_size = (int(2048 / 4), int(1080 / 4))
    fps = 29
    MAX_DURATION = 4
    out_path = Path("data/Images/Greenscreen_Video_frames_4sec_mini") / split
