...
class Backgrounds(data.Dataset):
    ...
    def __init__(self, transforms: list = None, norm_ImageNet=False, train=True, start_index=torch.tensor([0])):
        ...
        with open("data/Images/other/rgb_dataset/" + self.mode + "/out_log.json", "r") as json_file:
    ...
    def __getitem__(self, idx):
        ...
        lbl = Image.open(str(Path.cwd() / Path(self.data["labels"][0])))
    ...
    def show(self, num_images, start_idx: int = 0, random_images=True):
        ...
        for i in range(start_idx, num_images):
            ...
            lbl = Image.open(self.data["labels"][0])
...
if __name__ == "__main__":
    ...
    dataset = Backgrounds(train=False)
